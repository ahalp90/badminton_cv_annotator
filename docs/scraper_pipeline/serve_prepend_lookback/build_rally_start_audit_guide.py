...
The generated rows are annotation proposals and pending event records. They do
not change the canonical broadcast timelines or production annotator output.
"""
...
def build_audit_rows(repo_root: Path, controls_per_video: int = DEFAULT_CONTROLS_PER_VIDEO) -> tuple[list[dict[str, object]], list[Path]]:
    ...
    rows: list[dict[str, object]] = []
    input_paths: list[Path] = [master_path, SCRIPT_PATH]
...
def write_audit_package(
    ...
    """Write deterministic per-video target and pilot files.
    ...
    for fixture in FIXTURES:
        ...
        output_paths.extend((target_path, pilot_path))
        per_video[fixture.name] = {
            ...
            "unknown_first_type": sum(row["gt_first_type_en"] == "unknown" for row in fixture_rows),
        }
    summary: dict[str, object] = {
        ...
        "guardrails": [
            ...
            "all human event fields start pending and remain separate from canonical timelines",
...
def main() -> int:
    args = _parse_args()
    summary = write_audit_package(args.repo_root, args.out, args.controls_per_video)
    counts = summary["counts"]
    assert isinstance(counts, dict)
    print(
        f"wrote {counts['pooled_targets']} target rows and "
        f"{counts['pooled_pilot_rows']} pilot rows to {args.out}"
    )
    return 0
