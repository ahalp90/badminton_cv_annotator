...
> the CPU gates (G1-G6, raw-schema). The 2026-07-02 as-built adversarial review
> (Batches 0-3) is folded into the worklog; `04_adversarial_review.md` is the
> separate 2026-07-01 round-1 plan review. The GPU handoff
> (G7 self-variance, G8 parity, G9 Phase-A) ran on Bourbaki: Phase-A GO at
> DET_SCORE_THR=0.15 (see `06_phase_a_decision.md`). Merging as PR #4. Line
> numbers are a read-time snapshot; locate by symbol.
...
+ RTMPose-L COCO-17, updated `body7` weights), with the downstream code path
unchanged (byte-identical on fixed inputs) and verified extraction parity on the
live output.
...
- **Downstream is model-agnostic**: it depends only on the COCO-17 schema +
  ...
  is behaviourally transparent to downstream *provided* the adapter reproduces
  the per-person contract.
- **Migrate, do not delete.** Under usage uncertainty, migrating the 2D paths is
  low-risk: it preserves any manual/external use and removes mmpose. 3D is
...
- **Model is updated**, so raw keypoints will not byte-match the old extract.
  Acceptance is staged: CPU downstream byte-identity plus GPU extraction parity,
  then a decision gate, then full re-extract, retrain, and F1 compare.
...
- The entire `src/api/` layer, BRIC (`bric_inference.py`, `perception/players.py`
  via ultralytics YOLO), BST-X live inference (precomputed tensors), TrackNetV3
...
1. `raw_extract.py` (GPU) produces `ShuttleSet_keypoints_raw/` (32,203 stems).
2. `apply_heuristic.py --heuristic sticky_anchor` (CPU) produces
...
(see §1). Cannot fully rule out manual/external use of Step 2, so migrate,
don't delete.
...
- **Finding 1: solutions return only `(keypoints, scores)`; no bbox / no
  ...
  `RTMDet.postprocess` to also return the kept scores, which it already computes).

- **Finding 2: undocumented BGR/RGB bug in rtmlib's RTMPose preprocessing.** It
  uses an RGB-order mean but never converts BGR to RGB, effectively swapping R/B vs
...
- **Empty-frame trap:** rtmlib's RTMPose falls back to a full-image bbox on zero
  detections, yielding one spurious full-frame "person". The adapter must guard:
  zero detections give `ndet=0`, an all-NaN frame (matching mmpose).

- **Determinism:** see `03_verification.md` (its home): CPU EP is deterministic
  for fixed input/model/thread-count, CUDA EP is not, and CPU differs from CUDA
  bit-for-bit; proven by G4.

## 5. Model + dependency decisions (locked)

- **Pose:** `rtmpose-l_simcc-body7_pt-body7_420e-256x192-4dba18fc_20230504`
  (RTMPose-L, COCO-17, 256x192).
- **Detector:** `rtmdet_nano_8xb32-100e_coco-obj365-person-05d8511e` (320x320,
  NMS baked in).
...
7. The `_order_two_on_court` helper and the equivalence contract between
   `current` and `detect_players_2d` (B5 invariants in `function_invariants/detect_players.md`): strict-`>` y-flip, `<2` short-circuit before `check_pos_in_court`, `dtype=float` (float64) appends.
...
| `ShuttleSet_keypoints_raw/` | 32,203 clips x 5 raw arrays (N_max=16) | CPU byte-equality fixtures; mmpose parity baseline |
| `ShuttleSet_keypoints_clean_sticky_anchor/` | 32,203 clips x 3 clean arrays | deployed baseline (never overwrite) |
...
Local host: CPU-only (6 cores, 46 GB, numpy 2.2.4). Runs the raw-to-clean CPU gate
and rtmlib CPU inference. GPU extraction + retrain run remotely (Bourbaki); user
executes those (no SSH from here).

## 8. Acceptance criteria: staged (parity, then retrain gate)
...
**Phase B (full):** full 30,487-stem re-extract (the mmpose baseline covered
32,203 clips), then re-run `sticky_anchor`, re-collate, retrain BST-X, and
compare macro-F1 / min-class-F1 against the committed baseline. Mirrors the prior
mmpose-heuristic Phase-1-to-Phase-2. (Phase A has cleared, GO at 0.15; see
...
| D2 | Acceptance bar | Staged: parity, then retrain decision gate | 2026-07-01 |
| D3 | Pose model | RTMPose-L body7 @ 256x192; detector rtmdet-nano-person | 2026-07-01 |
...
- O4: whether to also expose an `--input-size 384x288` option for a later
  accuracy pass (not now; 256x192 is the baseline).
