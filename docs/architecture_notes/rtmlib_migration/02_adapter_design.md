...
| `bbox` → `bbox[0]` | `(4,)` xyxy | raw_extract, detect_players_2d |
| `bbox_score` | scalar | raw_extract (→ `_raw_scores`, consumed by sticky_anchor) |
| `keypoint_scores` | `(17,)` | raw_extract (→ `_raw_kp_scores`, consumed by no heuristic) |
...
1. **`RTMDetScored(rtmlib.RTMDet)`** — the person ONNX has NMS baked in
   ...
   `(boxes (M,4) f32, scores (M,) f32)` for boxes with `score>0.3`.
...
2. **RGB-fixed `RTMPose`** — rtmlib's RTMPose uses an RGB-order mean but never
   converts BGR→RGB (undocumented bug). The adapter feeds it
...
Models: detector `rtmdet_nano...05d8511e` @ 320×320 (hash-identical to mmpose's);
...
3. **Empty-frame guard:** zero detections ⇒ `n=0`, all-NaN frame. rtmlib's
   RTMPose would otherwise fabricate a full-image "person" — must be blocked.
...
6. **detect_players_2d side (as-built):** the loop now reads `det.keypoints` /
   ...
   **G3**, verified decisive by a float64-vs-float32 negative control). The
   ...
   `normalize_joints` — reading the adapter arrays directly is cleaner and
   trap-free. Detection *order* is not reproduced (rtmlib NMS order ≠ RTMDet
...
onnxruntime CPU EP is deterministic run-to-run for fixed input+model+thread
count; CUDA EP is not, and CPU≠CUDA bit-for-bit. This only affects the
inference-running gates (Tier 1/3), never the pure-numpy downstream (Tier 2).
rtmlib's `BaseTool` builds the `InferenceSession` with no `SessionOptions`, so
`intra_op_num_threads` is not a constructor knob; a gate that needs a fixed
thread count sets `OMP_NUM_THREADS=1` in its run command (G4 does, and proves it
bit-identical). CUDA (Phase B) is nondeterministic regardless.
...
rtmlib's person ONNX is fixed at 320×320. The extra boxes are low-salience
...
detector to match density (different detector, more boxes). Default (a) pending
the parity numbers.
...
**This settles the `ndet` concern.** Despite rtmlib returning a smaller,
cleaner detection set, `sticky_anchor` selects the **same two players on ~99% of
frames**, and where both keep a frame the positions/joints agree to <1% of the
court / ~2% of the bbox diagonal. The extra mmpose crowd detections were noise
`sticky_anchor` rejected; rtmlib's set already contains the two salient players.
So `rtmdet-nano @ 320` (hash-identical detector) is sufficient — no need to swap
to a higher-res YOLOX. The deployed output is preserved; the residual is
model-noise the retrain (Phase B) will confirm at F1 level.
...
mmpose kept; failed-agreement mean **0.981** (min 0.857 on a hard busted clip);
