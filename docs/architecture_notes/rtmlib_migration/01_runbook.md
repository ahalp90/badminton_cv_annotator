...
> The assistant made no commits; the user committed the tree with the drafted
> messages, now merged as PR #4.
...
- **Files (new, under `src/bst_x/validation_scripts/rtmlib_migration/`):**
  ...
  - `gate_cpu_downstream_byteeq.py` (**G5**): dual-invocation precondition (fixed mmpose raw fed through unchanged downstream).
...
- **Depends on:** nothing (G1/G2/G4/G6 need the Batch-1 adapter to gate against, so author the harness here and wire to the adapter in Batch 1).
...
- User runs rtmlib `raw_extract` on `smoke50` + 1-2 shards on Bourbaki (GPU or CPU); `gate_gpu_parity.py` + a CPU `apply_heuristic sticky_anchor` on the new raw for failed-rate vs baseline. Decision gate (thresholds O3). No src edit.

### Batch 6: Phase-B full re-extract + retrain (HALT-AND-HANDOFF, Phase A cleared, GO at 0.15)

- Full 30,487-stem re-extract, then `sticky_anchor`, collate, retrain BST-X, then macro/min-F1 vs committed. No src edit; consumes the migrated code.

## Commit messages (approved and used for the PR #4 commits; no `Co-Authored-By: Claude` trailer)
...
> separate docs commit, not part of C0-C4. All five were applied per-batch (the
> tree carried Batches 0-4 mixed) and committed as PR #4.
...
wording mmpose to rtmlib, and gitignore the G7/G8 gate JSON dumps.
```
...
- rtmlib caches models to `~/.cache/rtmlib/hub/checkpoints/` (override `XDG_CACHE_HOME`); no SHA check by default, so pin + verify in Batch 0.
...
- Pool env: local `python3` lacks pandas; the CPU raw-to-clean gate needs a venv with numpy + pandas. (Originally cautioned pandas<3 for the codebase's inplace/iloc patterns; G5 then verified byte-identical downstream on pandas 3.0.3 over 50 clips, so the extraction venv pins 3.0.3.)
