...
0.3-to-0.15 recalibration (06): all compensation for an unintended detector
...
mmengine 0.10.4, mmcv 2.1.0). At tag v1.3.2 (identical at v1.3.0/1.3.1, and
RTMDet-M already the choice at v1.2.0),
...
- The `human` alias *pose* model (the only metafile line defining the alias,
  ...
  not the RTMPose-L the migration-era docs had recorded.
...
Code delta (one commit): `rtmlib_pose.py` sets `DET_URL` to
`rtmdet_m_8xb32-100e_coco-obj365-person-235e8209.zip` (same
`rtmposev1/onnx_sdk/` bucket, uploaded 7 seconds after the nano zip, from the
same mmdeploy export batch with the same NMS-baked `dets (1,N,5)` + `labels`
output, so `RTMDetScored` is unchanged), `DET_INPUT_SIZE` to (640, 640), and
`DET_SCORE_THR` to 0.3. New SHA-256 pin (extracted `.onnx`, identical across
...
- **NMS:** mmpose ran a second NMS at IoU 0.3 after its score cut; the ONNX
  ...
  survive here that mmpose suppressed, giving a small positive ndet excess
...
**GO.** The authoritative 200-clip rerun, over the same stratified
`phase_a_stems.txt` as the nano-era G-4 (like-for-like), against the committed
mmpose raw:
