...
- **Per-class augmentation experiment**: Isiah's augmentation writeup at `docs/research/tri1_misc/Augmentation.pdf` proposes class-aware augmentation. If applied selectively to smash/ws/mid-tail classes (the ones with the train-test gap), it directly attacks the generalisation problem this analysis surfaces.
