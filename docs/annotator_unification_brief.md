...
2. **Post-fit reprojection gate, calibrated to point count** (`fit_corner_quad`, `FIT_MAX_REPROJ_PX = 3.0`). Ours gates on the WORST point, not the mean, because least squares smears one bad click across the good ones (a 30 px misidentification among six clicks leaves the mean near 1 px). BUT the 3.0 constant was tuned for minimal 5-6 point tour fits; your own frames show worst-point growing with landmark count (4 to 8.8 px across 18-25 points while corner repeatability stays 2-4 px). Recommended form for your tool: hard-fail at 3 px worst for fits of 6 points or fewer; above that, warn-and-confirm showing both worst and RMS rather than hard-failing. Do not adopt the bare 3 px constant; it would have rejected most of your real frames.
