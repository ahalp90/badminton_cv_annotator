...
OUT_DIR = HERE / 's27_sticky_gate_arm_r7_outputs'  # r7 variant: sticky arm suppressed at radius 7
GATE_THRESHOLD = stage8.BODY_UNIT_WRIST_THRESHOLD

# Late-bound in main() before any use; module-level binding keeps the name checker honest.
_HOMO_DF = None
_ALL_COURT_INFO = None
...
def main() -> None:
    ...
    for video in ('pilot', 'vid15'):
        ...
        ], radius=7))  # r7 variant: Ariel's recall-preferred radius; courtbox r7 comparator = the s26 matrix row
        sticky_contacts = [
            ContactCandidate(
