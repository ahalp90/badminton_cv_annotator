...
def build_chain(cfg, track, bboxes, scores, kps, ndet, dead, homo_df, court_info,
    ...
    `gate_res`) alongside the pose arrays and court box. `wrist_near` is still the
    gate-plus-suppression verdict.
    """
    ...
    filtered_contacts = [c for c in contacts if c[3] is not False]  # wrist_near is not False
    filtered_by_rally: dict[int, list[int]] = {}
