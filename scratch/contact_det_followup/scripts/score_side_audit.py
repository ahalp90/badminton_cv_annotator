...
DECISION_PATH = REPO_ROOT / "scratch/contact_det_followup/results/simple_side_decisions.json.gz"
...
def _section_outcomes(result: Mapping[str, object]) -> dict[tuple[str, int], str]:
    whole = result["whole_rallies"]
    rows = whole["by_tolerance"]["5"]["sections"]
    return {
...
def _human_sides(saved_labels: SavedLabels) -> dict[tuple[str, str], tuple[str | None, ...]]:
    return {
        (fixture, f"{rally.set_id}:{rally.rally}"): tuple(contact.side for contact in rally.contacts)
        for fixture, rallies in saved_labels.labels.rallies_by_fixture.items()
        for rally in rallies
    }


def _rally_ids(result: Mapping[str, object]) -> dict[tuple[str, int], str]:
    rows = result["whole_rallies"]["by_tolerance"]["5"]["sections"]
    return {
        (str(row["fixture"]), int(row["span_id"])): str(row["rally_id"])
        for row in rows
    }


def _alternates(sides: tuple[str | None, ...]) -> bool:
    return bool(sides) and all(side is not None for side in sides) and all(
        left != right for left, right in pairwise(sides)
    )
...
def side_pattern_ceiling(
    ...
    outcomes = _section_outcomes(baseline_result)
    rally_ids = _rally_ids(baseline_result)
    human_sides = _human_sides(saved_labels)
    wrong = {identity for identity, outcome in outcomes.items() if outcome == "wrong_predicted_side"}
    unanswered = {identity for identity, outcome in outcomes.items() if outcome == "predicted_side_unanswered"}
    repairable_wrong = {
        identity
...
def _strict_correct(
    ...
    return fully_correct_ids(result) & strict_ids
...
def run_audit() -> dict[str, object]:
    ...
    baseline = score_streams(saved_labels.labels, predictions.spans, predictions.events_by_fixture)
    ...
    baseline_correct = _strict_correct(baseline, strict_ids)
    revised_correct = _strict_correct(revised, strict_ids)
    repaired = revised_correct - baseline_correct
    broken = baseline_correct - revised_correct
    baseline_side = baseline["player_side"]["5"]["total"]
    revised_side = revised["player_side"]["5"]["total"]
    timing = baseline["contact_timing"]["5"]["total"]
    contact_and_side_denominator = int(timing["labelled_contacts"]) + int(
        timing["predicted_contacts"]
    )
    ceiling = side_pattern_ceiling(baseline, saved_labels, strict_ids)
    return {
        ...
        "simple_vote": {
            ...
            "baseline_full_output_precision": len(baseline_correct) / len(predictions.spans),
            "revised_full_output_precision": len(revised_correct) / len(predictions.spans),
            "baseline_side_accuracy": float(baseline_side["accuracy_when_both_answered"]),
...
def main() -> None:
    ...
    print(json.dumps({"ceiling": payload["ceiling"], "simple_vote": payload["simple_vote"]}, indent=2))
