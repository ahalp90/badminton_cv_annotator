...
shortlist caps and the cascade were both rechecked with the filter. The caps
stay at 256. The cascade would now save only about 5–6%, so it is parked.
...
| Coarse scores, then exact scores for the top K only ([item 16](../archive/20260925_optimisation_handover/CLAUDE_FOLLOWUPS.md#16-tested-score-coarsely-then-exactly-only-the-top-k)). Set aside on 25 September, planned again on 26 September, then parked the same day | About half of court scoring at K = 2,048. With the upright-camera filter, that is about 5–6% of the run, estimated but not timed ([cascade_cost.txt](claude_evidence/built_courts/cascade_cost.txt)) | At K = 2,048, the same overall shortlists on all 23 views that reach a search, before the filter; not guaranteed on new views, and a miss gives no warning | Parked behind parallel work and court reuse ([plan and evidence](../webui_final_opt_handover/README.md#the-cascade-item-16-parked)) | No |
...
| Fully score only each pair's top K courts, ranked by the average of their two line-guess scores | The courts that matter sit about as deep by that average as by build order. Keeping every overall shortlist whole would need K = 113,937, which skips 0.04% of full scoring | [Check](../webui_final_opt_handover/README.md#ruled-out-choosing-the-k-courts-by-their-line-guess-average) | No |
...
| Larger axis-scoring batches on the CPU (F4) | Not faster. In a laptop benchmark of `score_axes` (26 September), batches of 1,024 and 4,096 took 0.99–1.77× as long as today's 256, and 16,384 up to 14× as long. Larger batches push the working arrays out of the CPU's cache. Peak memory is not the limit: 1.4–6.1 MiB per call at 256 and 21–97 MiB at 4,096. The right batch size on a GPU is still open | [Benchmark](claude_evidence/axis_batch/axis_batch.txt); [verdict](../archive/20260925_optimisation_handover/CLAUDE_EVALUATION.md#claim-by-claim-verdicts) | No. It keeps 256 |
