...
def write_pool(path: Path, records: list[tuple[int, int, np.ndarray, np.ndarray]]) -> None:
    """Store the proposed courts and retained indices of every matched pair as one compressed .npz."""
    arrays = {}
    for pair_id, count, corners, retained_index in records:
...
def generate(source: dict, saved: dict, zone: object, root: Path, pool_path: Path | None = None) -> dict:
    ...
    # One entry per matched pair: pair ID, proposed count, corners (working px) and retained indices.
    pool_records: list[tuple[int, int, np.ndarray, np.ndarray]] = []
