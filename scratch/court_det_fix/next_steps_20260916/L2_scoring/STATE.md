...
- Inputs: `line_identity/inputs/{baseline,paint_observations}/{cases,estimators}/`, the saved
  matcher records and logs under `line_identity/runs/line_identity_20260915_222437/`, the
...
- Completed: the replay uses shared S0/S1 stripe caches, preserves G0/G1/U order and origin keys,
  ...
  or scientific arm was added. Serena semantic tools are active, although the launcher status
  command cannot read its state lock in this sandbox.
- Current result: all 24 comparison cells are present. All eight saved diagonal winner identities
  match. Each of the four diagnostic traces reproduces both the saved aggregate profile and the
  existing implementation, including every passing centre shift and the detector interval mask.
  A local Am3 numerical mismatch was reproduced exactly on Carmack, so the exact-runtime result is
  the authoritative replay for that case.
- Interpretation: candidate access changes every fixed-case winner family under S0; rescoring alone
