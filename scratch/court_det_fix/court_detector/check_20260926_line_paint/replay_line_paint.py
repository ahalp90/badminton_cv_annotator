"""Replay the court detector's final choice with and without line-averaged paint, from the upright check's results.

The line-averaged paint acts only in the net choice, after W5 scoring. So the saved W5 record and feet,
with the view's frame, line fragments and person boxes, give what a full run would. For each view this
rebuilds the scoring context, then calls the detector's own choose_court once per arm, with self-checks on.
The arms are W5's paint alone, as the upright check ran; today's default, the 10% geometry blend; and the
blend with line-averaged paint. The paint-alone arm must reproduce the saved final court: the same chosen
court and outcome, and corners within 0.0001 native px.
...
Usage, from the repository root:
  python -m scratch.court_det_fix.court_detector.check_20260926_line_paint.replay_line_paint UPRIGHT_RUN OUT
...
ARMS = {
    "paint_only": Switches(geometry_weight=0.0),
    "blend": Switches(),
    "line_paint": Switches(line_paint=True),
}
TOLERANCE_NATIVE_PX = 1e-4


def reproduction_problem(result_row: dict, saved: dict) -> str | None:
    """Why the paint-only arm's result differs from the saved one, or None."""
            ...
            for arm, switches in ARMS.items():
                ...
                if arm == "paint_only":
                    ...
                    if problem is not None:
                        problems.append(f"{view_id} paint_only: {problem}")
