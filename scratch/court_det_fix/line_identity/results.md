...
- The axis matching's per-direction cap discards the court's own matching when many parallel non-court lines compete, because its score rewards any five tightly aligned lines and the cap keeps 512 of tens of thousands. Filtering the fragments the axis matching sees, with directions held fixed, is the first change in this series that moves views without a lottery: at the generation stage it repairs both Amateur-2 views' winners and leaves the broadcast views within 2.2 px, at a cost on GX0 and Amateur-3 of one to five pixels on the nearest pooled court and the line winner and up to 13 on the paint winner. The filter removes some marking fragments on those two views; whether that is the cause was not measured.
