...
pixels of the fragment is than the brighter of the two flank means, taken over FLANK_WORKING_PX
on each side) and the saturation at that brightest pixel. Court paint is a bright, unsaturated
...
Fragments are labelled for the study only: "edge" when both endpoints lie within EDGE_PX working
pixels of the control court's outline, "offending" for GX0's three rows the earlier check
identified as non-court structures, "other" for the rest. The labels come from the control and
are never used by any filter. Writes runs/<out>/profiles.csv.gz (one row per fragment per view)
and prints quantiles per label.
"""
...
def features(profile: np.ndarray) -> np.ndarray:
    ...
    left_flank = (offsets <= -FLANK_WORKING_PX[0]) & (offsets >= -FLANK_WORKING_PX[1])
    right_flank = (offsets >= FLANK_WORKING_PX[0]) & (offsets <= FLANK_WORKING_PX[1])
    # A ridge must fall away on both sides, so the contrast is measured against the brighter flank.
    flank = np.maximum(brightness[..., left_flank].mean(axis=-1), brightness[..., right_flank].mean(axis=-1))
...
def study_case(case_id: str) -> list[dict]:
    ...
    start_distance = edge_distance(segments_working[:, :2], control_working)
    end_distance = edge_distance(segments_working[:, 2:], control_working)
    on_edge = (start_distance <= EDGE_PX) & (end_distance <= EDGE_PX)
    ...
    for fragment_id, (contrast, peak_saturation, peak_offset, centre, flank) in enumerate(measured):
        study_label = offending.get(fragment_id, 'edge' if on_edge[fragment_id] else 'other')
...
def main() -> None:
    ...
    for case_id in CASE_IDS:
        ...
        for study_label in ('edge', 'other'):
            subset = [row for row in case_rows if row['study_label'] == study_label]
            print(f'  {study_label:5s} ridge contrast {quantiles([row["ridge_contrast"] for row in subset])}')
            print(f'  {study_label:5s} peak saturation {quantiles([row["peak_saturation"] for row in subset])}')
