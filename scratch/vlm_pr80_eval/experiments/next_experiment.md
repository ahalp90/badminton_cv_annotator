...
- Develop the rule on `sset_01` and `sset_15`.
- Keep `sset_21` untouched until the rule and thresholds are frozen.
...
all thresholds beside the file. Do not choose thresholds after looking at
`sset_21`.
...
rule is frozen, repeat preparation and scoring with only `--video sset_21` and
the frozen rule file:

```bash
PYTHONPATH=scratch/vlm_pr80_eval:src \
~/.venvs/badminton-cicd/bin/python -m experiments.score_contact_model_cleanup \
  --cases SAFETY_ROOT/cases \
  --attempts SAFETY_ROOT/attempts/intern01 \
  --artifacts-root ISSUE103_ARTIFACTS \
  --frozen-rule TRIAL_ROOT/scores/frozen-rule.json \
  --out SAFETY_ROOT/scores/safety.json
```
...
version, development input hashes, and creation command. The safety scorer
must copy that whole object into its output before applying it.
...
On the two development videos, try contact-score thresholds from 0 to 1 in
...
one rule before the safety run.

### 5. Run the safety set once

Run the frozen contact-only baseline and frozen combined rule on `sset_21`.
Report both. Do not retune after seeing this result.
...
- the combined rule gains precision only by discarding most otherwise usable
  rallies;
- a model or prompt change is needed after looking at `sset_21`.
