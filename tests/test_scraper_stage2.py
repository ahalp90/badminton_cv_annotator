"""Stage 2 (transcript acquisition) unit tests. CPU-only, no network.
...
Run from repo root::
    ...
    ~/.venvs/badminton-cicd/bin/python -m pytest tests/test_scraper_stage2.py -v
"""
import json
...
from src.scraper import config, stage2_transcripts as stage2
...
def test_parse_json3_skips_timing_only_and_joins_segs(tmp_path):
    ...
    segments = stage2.parse_json3(path)
...
def test_parse_vtt_joins_multiline_and_skips_empty_cues(tmp_path):
    ...
    segments = stage2.parse_vtt(path)
...
def test_pull_subtitles_prefers_json3_over_vtt(tmp_path, monkeypatch):
    ...
    path = stage2.pull_subtitles('vid', 'https://y/vid', str(work))
...
def test_pull_subtitles_none_when_no_caption_file(tmp_path, monkeypatch):
    ...
    assert stage2.pull_subtitles('vid', 'https://y/vid', str(work)) is None
...
def test_whisperx_fallback_none_when_whisperx_missing():
    ...
    assert stage2.whisperx_fallback('vid', 'https://y/vid') is None


# ---------------------------------------------------------------------------
# run_stage2 batch behaviour
# ---------------------------------------------------------------------------
@pytest.fixture
def stage2_env(tmp_path, monkeypatch):
    ...
    transcripts.mkdir()
    monkeypatch.setattr(stage2, 'TRANSCRIPTS_DIR', transcripts)
    monkeypatch.setattr(stage2, 'check_ytdlp', lambda: None)
    monkeypatch.setattr(stage2, 'ensure_dirs', lambda: None)
    monkeypatch.setattr(stage2.time, 'sleep', lambda *a, **k: None)
    monkeypatch.setattr(stage2.random, 'uniform', lambda a, b: 0.0)
    return transcripts
...
def test_stage2_circuit_breaker_fires_at_floor(stage2_env, monkeypatch):
    calls = {'n': 0}
    ...
    monkeypatch.setattr(stage2, 'acquire_transcript', always_fail)
    with pytest.raises(RuntimeError, match='block threshold'):
        stage2.run_stage2(_rows(20))
    # Stops exactly at the floor rather than burning all 20 attempts.
    assert calls['n'] == config.STAGE2_BLOCK_MIN_ATTEMPTS


def test_stage2_below_floor_no_early_stop_then_final_block(stage2_env, monkeypatch):
    calls = {'n': 0}
    ...
    monkeypatch.setattr(stage2, 'acquire_transcript', always_fail)
    below_floor = config.STAGE2_BLOCK_MIN_ATTEMPTS - 1
    with pytest.raises(RuntimeError, match='block threshold'):
        stage2.run_stage2(_rows(below_floor))
...
def test_stage2_no_block_at_or_below_fraction_and_writes_sidecars(stage2_env, monkeypatch):
    transcripts = stage2_env
    ...
    monkeypatch.setattr(stage2, 'acquire_transcript', acquire)
    stage2.run_stage2(_rows(8))

    written = sorted(path.name for path in transcripts.glob('*.json'))
    assert written == ['v0.json', 'v2.json', 'v4.json', 'v6.json']


def test_stage2_resume_skips_existing_sidecars(stage2_env, monkeypatch):
    transcripts = stage2_env
