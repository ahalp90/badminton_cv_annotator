...
def test_partial_lane_rejects_alternating_cross_slot_minimum() -> None:
    setup = _setup(1.0)
...
def test_standard_lane_accepts_either_slot_when_its_ratio_passes() -> None:
    setup = _setup(2.0)._replace(wrist_dist=np.full((4, 2), (0.02, 0.8)))
    assert _sticky_serve_setup_before(setup, 3, 0.3, 4, None, None)


def test_standard_lane_pairs_each_distance_with_its_own_height() -> None:
    setup = _setup(2.0)._replace(
...
def test_sticky_gate_ignores_invisible_corner_garbage() -> None:
    setup = _setup(1.0, n=6)
...
def test_sticky_distance_window_excludes_burst_frame() -> None:
    setup = _setup(1.0)
    wrist_dist = np.full((4, 2), 0.4)
    wrist_dist[3] = 0.01
...
def test_burst_frame_count_cannot_change_lane_selection() -> None:
    setup = _setup(1.0, n=3)._replace(
...
def test_burst_analysed_row_is_ignored_without_stillness_and_required_with_it() -> None:
    setup = _setup(2.0)._replace(
...
def test_empty_setup_window_returns_before_downstream_medians(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert not _sticky_serve_setup_before(_setup(1.0), 0, 0.3, 4, None, None)


def test_standard_lane_presence_floor_keeps_absent_slot_from_being_rescued() -> None:
    setup = _setup(2.0)._replace(wrist_dist=np.column_stack([np.full(4, 0.02), np.full(4, np.nan)]))
    assert not _sticky_serve_setup_before(setup, 3, 0.3, 4, None, None)


def test_standard_lane_stillness_rejects_a_player_even_when_distance_passes() -> None:
